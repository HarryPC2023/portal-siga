...
function inicializar(cursos) {
    ...
    cursos.forEach((c, i) => {
        ...
        // También se registra bajo el nombre "limpio" (sin el sufijo interno
        // de código): el calendario y el Excel exportado buscan el color
        // por ese nombre limpio, no por la llave interna cruda.
...
// ── NOMBRE A MOSTRAR (compartido con scheduler.js) ────────────
// La llave interna del curso a veces trae " (CÓDIGO)" pegado al final
// — solo pasa cuando dos cursos de carreras distintas comparten el
// mismo nombre oficial pero tienen código diferente (ver parser.js /
// generar_carga_horario.py). Internamente hace falta para no mezclarlos,
// pero en pantalla se quita: el código ya se ve aparte en el badge.
    ...
    Object.entries(data).forEach(([curso, secMap]) => {
      ...
      <div class="course-chevron">▶</div>`;
        ...
        secciones.forEach(sec => {
            ...
            const vacantes = secMap[sec].vacantes;
            ...
            // vacantes: cupo oficial de la sección al momento de la carga
            // horaria (no se actualiza en vivo durante la matrícula)
            const vacHtml = (vacantes !== undefined && vacantes !== null)
                ? ` <span class="badge-cupos" title="Cupos oficiales al momento de la carga">${vacantes} cupos</span>`
                : '';
            span.innerHTML = `<strong>Sección ${sec}</strong> — ${docente}${vacHtml}`;
