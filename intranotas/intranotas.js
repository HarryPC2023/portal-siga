...
function calcularTodo() {
    ...
    if (document.getElementById('prediccion-aa-resultado')) refrescarVistaPrediccion();
...
/* ============================================================
   ...
   tarjeta). Dentro viven 4 herramientas, las 4 funcionales:

     🎯 Meta del curso     — motor de escenarios (¿qué necesito?)
     🔮 Predicción         — proyección por promedio (¿a dónde voy?)
     🏆 Rendimiento/curso  — PF de todos los cursos de un ciclo
     📈 Evolución general  — ponderado ciclo a ciclo en el tiempo

   Meta y Predicción trabajan sobre el ciclo ABIERTO en Pantalla 4
   (leen cursosSeleccionados + el DOM en vivo, se recalculan solas
   con cada input vía calcularTodo() — ver el hook al final de esa
   función). Rendimiento y Evolución, en cambio, trabajan sobre
   CUALQUIER ciclo guardado (vía leerDatosPeriodos()), así que
   Rendimiento tiene su propio selector de ciclo independiente del
   que está abierto ahora mismo.

   Predicción y Rendimiento se activan con 1+ ciclo cargado;
   Evolución necesita 2+ ciclos — la detección corre con
   contarCiclosConDatos(). Como leerDatosPeriodos() ya está
   namespaceada por malla, la detección de "cuántos ciclos tienen
   datos" corre sobre la malla activa en cada momento (misma malla
   que ve Intranotas).
   ============================================================
   ============================================================ */
...
/* ============================================================
   🔮 PREDICCIÓN — proyección automática por curso

   A diferencia de Meta del curso (tú fijas la meta y el motor
   busca cómo llegar), acá no hay input del usuario: se toma el
   PROMEDIO de lo que ya ingresó en ese curso y se usa como valor
   proyectado para cada componente que aún le falta, respetando el
   mismo layout de componentes (COMPONENT_LAYOUT, sin ES) que ya
   usa Meta. Solo se activa por curso cuando hay 2+ notas cargadas
   — con 1 sola nota no hay ritmo real que proyectar. Trabaja sobre
   el ciclo ABIERTO en Pantalla 4 (mismo criterio que Meta), así
   que se refresca sola vía el hook en calcularTodo().
   ============================================================ */
let prediccionCursoSeleccionadoId = null;

/* Cursos del ciclo abierto que ya tienen 2+ notas ingresadas en su
   tarjeta — son los únicos elegibles para Predicción. */
function cursosElegiblesPrediccion() {
    return cursosSeleccionados.filter(curso => {
        const { todos } = obtenerComponentesCurso(curso);
        const actuales = leerValoresActualesCurso(curso);
        return todos.filter(c => actuales[c] !== null).length >= 2;
    });
}

function generarPrediccionCurso(curso) {
    const { todos } = obtenerComponentesCurso(curso);
    const actuales = leerValoresActualesCurso(curso);
    const ingresados = todos.filter(c => actuales[c] !== null);

    if (ingresados.length < 2) return { insuficiente: true };

    const suma = ingresados.reduce((acc, c) => acc + actuales[c], 0);
    const promedio = truncar(suma / ingresados.length, 1);

    const proyectado = { ...actuales };
    const pendientes = [];
    todos.forEach(c => {
        if (proyectado[c] === null) { proyectado[c] = promedio; pendientes.push(c); }
    });

    const { nota_final } = calcularPFCompleto(curso, proyectado);
    if (nota_final === null) return { insuficiente: true };
    ...
    return {
        ...
        pendientes,
        aprobaria: nota_final >= 10, // mismo umbral que ya usa calcularTodo() para "en riesgo/desaprobado"
    };
}

/* ---------- Vista de Predicción dentro del panel ---------- */
function generarVistaPrediccion() {
    if (!cursosSeleccionados.length) {
        return `
            <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
            <p class="aa-vacio">Selecciona tus cursos en Intranotas para poder usar Predicción.</p>
        `;
    }

    const elegibles = cursosElegiblesPrediccion();
    if (!elegibles.length) {
        return `
            <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
            <p class="aa-vacio">Ingresa al menos 2 notas en algún curso para ver tu proyección.</p>
        `;
    }

    const elegiblesOrdenados = [...elegibles].sort((a, b) => a.name.localeCompare(b.name));
    if (!prediccionCursoSeleccionadoId || !elegiblesOrdenados.some(c => c.id === prediccionCursoSeleccionadoId)) {
        prediccionCursoSeleccionadoId = elegiblesOrdenados[0].id;
    }

    return `
        <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
        <div class="meta-aa-selector">
            <label for="prediccionAACursoTrigger">Curso</label>
            <div class="campo-select-custom meta-curso-select-custom" style="width:100%;">
                <button type="button" class="select-custom-trigger" id="prediccionAACursoTrigger"
                    aria-haspopup="listbox" aria-expanded="false">
                    <span id="prediccionAACursoTriggerTexto"></span>
                    <span class="select-custom-chevron" aria-hidden="true">▾</span>
                </button>
                <ul class="select-custom-lista meta-curso-select-lista" id="prediccionAACursoLista" role="listbox" hidden></ul>
                <input type="hidden" id="prediccionAACursoValor">
            </div>
        </div>
        <div id="prediccion-aa-resultado"></div>
    `;
}

function inicializarVistaPrediccion() {
    const elegibles = cursosElegiblesPrediccion();
    if (!elegibles.length) return;

    const elegiblesOrdenados = [...elegibles].sort((a, b) => a.name.localeCompare(b.name));
    const cursoActual = elegiblesOrdenados.find(c => c.id === prediccionCursoSeleccionadoId);

    inicializarSelectPersonalizado({
        triggerId: 'prediccionAACursoTrigger', textoId: 'prediccionAACursoTriggerTexto',
        listaId: 'prediccionAACursoLista', valorId: 'prediccionAACursoValor',
        opciones: elegiblesOrdenados.map(c => ({ value: c.id, label: c.name })),
        alElegir: (valor) => cambiarCursoPrediccion(valor),
    })?.establecer(prediccionCursoSeleccionadoId, cursoActual ? cursoActual.name : '');

    refrescarVistaPrediccion();
}

function cambiarCursoPrediccion(cursoId) {
    prediccionCursoSeleccionadoId = cursoId;
    refrescarVistaPrediccion();
}

function refrescarVistaPrediccion() {
    const cont = document.getElementById('prediccion-aa-resultado');
    if (!cont) return;

    const curso = cursosSeleccionados.find(c => c.id === prediccionCursoSeleccionadoId);
    if (!curso) { cont.innerHTML = ''; return; }

    const r = generarPrediccionCurso(curso);
    if (r.insuficiente) {
        cont.innerHTML = `<p class="aa-vacio">Ingresa al menos 2 notas en este curso para ver la proyección.</p>`;
        return;
    }

    cont.innerHTML = `
        <div class="meta-aa-tarjeta">
            <div class="meta-aa-tarjeta-nombre">Tu promedio actual: ${r.promedio.toFixed(1)}</div>
            <p class="meta-aa-tarjeta-desc">
                ${r.pendientes.length
            ? `Si mantienes ese ritmo en lo que te falta (${r.pendientes.join(', ')}), tu PF proyectado sería:`
            : 'Ya tienes todas tus notas ingresadas — este es tu PF real, no una proyección:'}
            </p>
            <div class="meta-aa-completo ${r.aprobaria ? 'ok' : 'no'}">
                PF ${r.pendientes.length ? 'proyectado' : 'final'}: <strong>${r.notaFinal.toFixed(1)}</strong>
                ${r.aprobaria ? ' ✅ Aprobarías' : ' ❌ Desaprobarías'}
            </div>
            ${r.pendientes.length ? `<p class="meta-aa-tarjeta-desc" style="margin-top:10px;margin-bottom:0;">⚠️ Esto es una proyección basada en tu promedio actual, no una garantía.</p>` : ''}
        </div>
    `;
}

/* ============================================================
   🏆 RENDIMIENTO POR CURSO — PF de todos los cursos de un ciclo

   Tiene su propio selector de CICLO (a diferencia de Meta y
   Predicción, que solo ven el ciclo abierto): recorre cualquier
   periodo guardado vía leerDatosPeriodos() y recalcula el PF de
   cada curso con calcularPFCompleto() como caja negra — sin tocar
   el DOM de Pantalla 4, así puede mostrar un ciclo distinto al que
   tienes abierto. Excepción: si el ciclo elegido es justo el que
   está abierto ahora mismo, lee el DOM en vivo en su lugar (evita
   mostrar notas desactualizadas mientras aún no guardas).
   ============================================================ */
let rendimientoCicloSeleccionado = null;

/* "2026-2" -> comparable numérico para ordenar cronológicamente.
   Reutilizado también por Evolución del promedio. */
function compararPeriodos(a, b) {
    const [anioA, numA] = a.split('-').map(Number);
    const [anioB, numB] = b.split('-').map(Number);
    return anioA !== anioB ? anioA - anioB : numA - numB;
}

/* Ciclos de la malla activa que tienen al menos una nota cargada,
   ordenados cronológicamente ascendente (el más antiguo primero). */
function obtenerCiclosConDatosOrdenados() {
    const datos = leerDatosPeriodos();
    return Object.keys(datos)
        .filter(p => {
            const entrada = datos[p];
            if (!entrada || !entrada.notas) return false;
            return Object.values(entrada.notas).some(notasCurso =>
                notasCurso && Object.values(notasCurso).some(v => v !== null && v !== '' && v !== undefined)
            );
        })
...
/* Valores de un curso para un periodo dado: si ese periodo es el
   que está abierto en Pantalla 4 ahora mismo, prioriza el DOM en
   vivo (más reciente que lo guardado); si no, lee lo guardado. */
function leerValoresPeriodo(periodo, curso) {
    if (periodo === periodoSeleccionado) {
        const { todos } = obtenerComponentesCurso(curso);
        const hayDom = todos.some(c => document.getElementById(`input-${curso.id}-${c}`));
        if (hayDom) return leerValoresActualesCurso(curso);
    }
    const datos = leerDatosPeriodos();
    const entrada = datos[periodo];
    return (entrada && entrada.notas && entrada.notas[curso.id]) || {};
}

function colorPorNota(nota) {
    if (nota >= 14) return '#10b981';
    if (nota >= 10) return '#2196f3';
    return '#ef4444';
}

function calcularRendimientoCiclo(periodo) {
    const datos = leerDatosPeriodos();
    const entrada = datos[periodo];
    if (!entrada || !entrada.cursos) return null;

    let sumaPonderada = 0, sumaCreditos = 0;
    const filas = entrada.cursos.map(curso => {
        const valores = leerValoresPeriodo(periodo, curso);
        const { nota_final } = calcularPFCompleto(curso, valores);
        const { todos } = obtenerComponentesCurso(curso);
        const completo = todos.length > 0 && todos.every(c => valores[c] !== null && valores[c] !== undefined && valores[c] !== '');

        if (nota_final !== null) { sumaPonderada += nota_final * curso.credits; sumaCreditos += curso.credits; }
        return { nombre: curso.name, credits: curso.credits, notaFinal: nota_final, completo };
    }).filter(f => f.notaFinal !== null);

    filas.sort((a, b) => b.notaFinal - a.notaFinal);
    const ponderado = sumaCreditos > 0 ? truncar(sumaPonderada / sumaCreditos, 3) : null;

    // Impacto en el ponderado: cuánto se movería si ese curso no existiera.
    const conImpacto = filas.map(f => {
        const creditosResto = sumaCreditos - f.credits;
        const ponderadoSinEste = creditosResto > 0 ? (sumaPonderada - f.notaFinal * f.credits) / creditosResto : null;
        const delta = (ponderadoSinEste !== null && ponderado !== null) ? truncar(ponderado - ponderadoSinEste, 3) : null;
        return { ...f, delta };
    });

    const alFilo = filas.filter(f => Math.abs(f.notaFinal - 10) < 1);

    return { filas: conImpacto, ponderado, alFilo };
}

/* ---------- Vista de Rendimiento por curso dentro del panel ---------- */
function generarVistaRendimiento() {
    const ciclos = obtenerCiclosConDatosOrdenados();
    if (!ciclos.length) {
        return `
            <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
            <p class="aa-vacio">Carga notas en al menos un ciclo para ver esto.</p>
        `;
    }

    if (!rendimientoCicloSeleccionado || !ciclos.includes(rendimientoCicloSeleccionado)) {
        rendimientoCicloSeleccionado = ciclos[ciclos.length - 1]; // el más reciente por defecto
    }

    return `
        <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
        <div class="meta-aa-selector">
            <label for="rendimientoAACicloTrigger">Ciclo</label>
            <div class="campo-select-custom meta-curso-select-custom" style="width:100%;">
                <button type="button" class="select-custom-trigger" id="rendimientoAACicloTrigger"
                    aria-haspopup="listbox" aria-expanded="false">
                    <span id="rendimientoAACicloTriggerTexto"></span>
                    <span class="select-custom-chevron" aria-hidden="true">▾</span>
                </button>
                <ul class="select-custom-lista meta-curso-select-lista" id="rendimientoAACicloLista" role="listbox" hidden></ul>
                <input type="hidden" id="rendimientoAACicloValor">
            </div>
        </div>
        <div id="rendimiento-aa-resultado"></div>
    `;
}

function inicializarVistaRendimiento() {
    const ciclos = obtenerCiclosConDatosOrdenados();
    if (!ciclos.length) return;

    // Más reciente primero en la lista desplegable — es lo que más se consulta.
    const opciones = [...ciclos].reverse().map(p => ({ value: p, label: p }));

    inicializarSelectPersonalizado({
        triggerId: 'rendimientoAACicloTrigger', textoId: 'rendimientoAACicloTriggerTexto',
        listaId: 'rendimientoAACicloLista', valorId: 'rendimientoAACicloValor',
        opciones,
        alElegir: (valor) => cambiarCicloRendimiento(valor),
    })?.establecer(rendimientoCicloSeleccionado, rendimientoCicloSeleccionado);

    refrescarVistaRendimiento();
}

function cambiarCicloRendimiento(periodo) {
    rendimientoCicloSeleccionado = periodo;
    refrescarVistaRendimiento();
}

function refrescarVistaRendimiento() {
    const cont = document.getElementById('rendimiento-aa-resultado');
    if (!cont) return;

    const r = calcularRendimientoCiclo(rendimientoCicloSeleccionado);
    if (!r || !r.filas.length) {
        cont.innerHTML = `<p class="aa-vacio">Este ciclo todavía no tiene notas suficientes para calcular nada.</p>`;
        return;
    }

    const listaHtml = r.filas.map(f => `
        <div class="rendimiento-aa-fila">
            <div class="rendimiento-aa-fila-info">
                <div class="rendimiento-aa-fila-nombre">
                    ${f.nombre}${f.completo ? '' : ' <span class="rendimiento-aa-incompleto">(incompleto)</span>'}
                </div>
                <div class="rendimiento-aa-fila-credits">${f.credits} créditos</div>
            </div>
            <div class="rendimiento-aa-fila-nota" style="color:${colorPorNota(f.notaFinal)}">${f.notaFinal.toFixed(1)}</div>
        </div>
    `).join('');

    const conImpacto = r.filas.filter(f => f.delta !== null).sort((a, b) => b.delta - a.delta);
    const sube = conImpacto[0];
    const baja = conImpacto[conImpacto.length - 1];
    const mostrarImpacto = sube && baja && sube.nombre !== baja.nombre && (sube.delta > 0 || baja.delta < 0);

    const alFiloHtml = r.alFilo.length ? `
        <div class="rendimiento-aa-seccion-titulo">📍 Cursos al filo</div>
        ${r.alFilo.map(f => `<div class="rendimiento-aa-filo-item">${f.nombre} — <strong>${f.notaFinal.toFixed(1)}</strong></div>`).join('')}
    ` : '';

    cont.innerHTML = `
        ${r.ponderado !== null ? `<div class="rendimiento-aa-ponderado">Ponderado del ciclo: <strong>${r.ponderado.toFixed(3)}</strong></div>` : ''}
        ${mostrarImpacto ? `
            <div class="rendimiento-aa-impacto">
                ${sube.delta > 0 ? `<div>📈 Te sube más el ponderado: <strong>${sube.nombre}</strong> (+${sube.delta.toFixed(3)})</div>` : ''}
                ${baja.delta < 0 ? `<div>📉 Te baja más el ponderado: <strong>${baja.nombre}</strong> (${baja.delta.toFixed(3)})</div>` : ''}
            </div>
        ` : ''}
        <div class="rendimiento-aa-lista">${listaHtml}</div>
        ${alFiloHtml}
    `;
}

/* ============================================================
   📈 EVOLUCIÓN DEL PROMEDIO — ponderado ciclo a ciclo

   Recorre TODOS los ciclos con datos (2+) de la malla activa,
   recalcula el ponderado de cada uno reutilizando calcularPFCompleto()
   por curso (misma fórmula que ya usa calcularTodo() para el banner:
   Σnota×créditos / Σcréditos, truncado a 3 decimales). Resalta el
   mejor y el peor ciclo, y detecta rachas de mejora/deterioro
   consecutivas mirando desde el ciclo más reciente hacia atrás.
   No tiene selector propio: muestra la línea de tiempo completa.
   ============================================================ */
function calcularPonderadoCiclo(periodo) {
    const datos = leerDatosPeriodos();
    const entrada = datos[periodo];
    if (!entrada || !entrada.cursos) return null;
    ...
    return sumaCreditos > 0 ? truncar(sumaPonderada / sumaCreditos, 3) : null;
}

function calcularEvolucion() {
    const ciclos = obtenerCiclosConDatosOrdenados();
    const puntos = ciclos
        .map(periodo => ({ periodo, ponderado: calcularPonderadoCiclo(periodo) }))
        .filter(pt => pt.ponderado !== null);

    if (puntos.length < 2) return { insuficiente: true };

    let mejor = puntos[0], peor = puntos[0];
    puntos.forEach(pt => {
        if (pt.ponderado > mejor.ponderado) mejor = pt;
        if (pt.ponderado < peor.ponderado) peor = pt;
    });

    // Racha de mejora/deterioro consecutiva, mirando desde el ciclo
    // más reciente hacia atrás — se corta apenas cambia el signo.
    let racha = 0, tipoRacha = null;
    for (let i = puntos.length - 1; i > 0; i--) {
        const diff = puntos[i].ponderado - puntos[i - 1].ponderado;
        if (diff === 0) break;
        const tipo = diff > 0 ? 'mejora' : 'deterioro';
        if (tipoRacha === null) tipoRacha = tipo;
        else if (tipo !== tipoRacha) break;
        racha++;
    }

    return { insuficiente: false, puntos, mejor, peor, racha, tipoRacha };
}

/* ---------- Vista de Evolución del promedio dentro del panel ---------- */
function generarVistaEvolucion() {
    const ciclos = obtenerCiclosConDatosOrdenados();
    if (ciclos.length < 2) {
        return `
            <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
            <p class="aa-vacio">Necesitas datos de al menos 2 ciclos para ver tu evolución.</p>
        `;
    }

    return `
        <button type="button" class="aa-volver" onclick="abrirHerramienta('grid')">← Volver</button>
        <div id="evolucion-aa-resultado"></div>
    `;
}

function inicializarVistaEvolucion() {
    refrescarVistaEvolucion();
}

function refrescarVistaEvolucion() {
    const cont = document.getElementById('evolucion-aa-resultado');
    if (!cont) return;

    const r = calcularEvolucion();
    if (r.insuficiente) {
        cont.innerHTML = `<p class="aa-vacio">Necesitas al menos 2 ciclos con notas calculables para ver tu evolución.</p>`;
        return;
    }

    const barrasHtml = r.puntos.map(pt => {
        const alturaPct = Math.max(4, (pt.ponderado / 20) * 100);
        const esMejor = pt.periodo === r.mejor.periodo;
        const esPeor = pt.periodo === r.peor.periodo && r.mejor.periodo !== r.peor.periodo;
        return `
            <div class="evolucion-aa-barra-col">
                <div class="evolucion-aa-barra-valor">${pt.ponderado.toFixed(2)}</div>
                <div class="evolucion-aa-barra${esMejor ? ' mejor' : ''}${esPeor ? ' peor' : ''}" style="height:${alturaPct}%"></div>
                <div class="evolucion-aa-barra-label">${formatoPeriodoCorto(pt.periodo)}</div>
            </div>
        `;
    }).join('');

    const mismoValorTope = r.mejor.periodo === r.peor.periodo;
    const destacadosHtml = mismoValorTope ? `
        <div class="evolucion-aa-destacados">
            <div class="evolucion-aa-destacado">📊 Tu ponderado se mantuvo estable: <strong>${r.mejor.ponderado.toFixed(3)}</strong> en todos tus ciclos.</div>
        </div>
    ` : `
        <div class="evolucion-aa-destacados">
            <div class="evolucion-aa-destacado mejor">🏆 Mejor ciclo: <strong>${formatoPeriodoCorto(r.mejor.periodo)}</strong> (${r.mejor.ponderado.toFixed(3)})</div>
            <div class="evolucion-aa-destacado peor">⚠️ Ciclo más flojo: <strong>${formatoPeriodoCorto(r.peor.periodo)}</strong> (${r.peor.ponderado.toFixed(3)})</div>
        </div>
    `;

    const rachaHtml = r.racha >= 2 ? `
        <div class="evolucion-aa-racha ${r.tipoRacha}">
            ${r.tipoRacha === 'mejora' ? '📈' : '📉'} Llevas <strong>${r.racha}</strong> ciclos seguidos ${r.tipoRacha === 'mejora' ? 'mejorando' : 'bajando'} tu ponderado.
        </div>
    ` : '';
    ...
    cont.innerHTML = `
        ${destacadosHtml}
        ${rachaHtml}
        <div class="evolucion-aa-grafico">${barrasHtml}</div>
    `;
}